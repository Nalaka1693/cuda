...
Uses column major flattening of 2D arrays
...
__global__ void matMul(int *matC_cuda, int *matA_cuda, int *matB_cuda){
	...
	if(row < ROWS1 && col < COLS2){
		...
		int k;
		for(k=0;k<COLS1;k++){
			prod=prod+matA_cuda[k*ROWS1+row]*matB_cuda[col*ROWS2+k];
		}
		matC_cuda[col*ROWS1+row]=prod;	
